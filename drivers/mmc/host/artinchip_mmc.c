...
static void artinchip_mmc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	case MMC_POWER_UP:
		...
		if (slot->host->power_gpiod)
			gpiod_set_value(slot->host->power_gpiod, 1);
	...
	case MMC_POWER_OFF:
		...
		if (slot->host->power_gpiod)
			gpiod_set_value(slot->host->power_gpiod, 0);
...
static const struct mmc_host_ops artinchip_mmc_ops = {
	...
	.card_hw_reset         = artinchip_mmc_hw_reset,
...
{
	struct artinchip_mmc *host = timer_container_of(host, t, cmd11_timer);
...
{
	struct artinchip_mmc *host = timer_container_of(host, t, cto_timer);
	unsigned long irqflags;
	u32 pending;
...
{
	struct artinchip_mmc *host = timer_container_of(host, t, dto_timer);
	unsigned long irqflags;
	u32 pending;
...
static struct artinchip_mmc_board *artinchip_mmc_parse_dt(struct artinchip_mmc *host)
{
	...
	host->power_gpiod = devm_gpiod_get_optional(dev, "power",
						    GPIOD_OUT_HIGH);
	if (IS_ERR(host->power_gpiod)) {
		dev_err(dev, "could not get power-gpios\n");
		host->power_gpiod = NULL;
	}
...
static int artinchip_mmc_aic_execute_tuning(struct artinchip_mmc_slot *slot, u32 opcode)
{
	...
	dev_info(&slot->mmc->class_dev, "start_smpl tuning sample phase:%d\n", start_smpl);
	do {
		mci_writel(host, SDMC_TTMC, 0xfffffff);
		smpl = artinchip_mmc_aic_move_next_clksmpl(host);
		dev_info(&slot->mmc->class_dev, "tuning phase:%d\n", smpl);
		/*send tuning cmd*/
	...
	dev_info(&slot->mmc->class_dev, "get the best tuning value:%d\n", best_clksmpl);
	if (best_clksmpl >= 0) {
...
static void artinchip_mmc_remove(struct platform_device *pdev)
{
