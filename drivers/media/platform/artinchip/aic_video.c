...
static int aic_dvp_subdev_init_state(struct v4l2_subdev *subdev,
				     struct v4l2_subdev_state *state)
{
	struct v4l2_mbus_framefmt *fmt;

	fmt = v4l2_subdev_state_get_format(state, DVP_SUBDEV_SINK);
				    ...
				    struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY)
		subdev_fmt = v4l2_subdev_state_get_format(state, fmt->pad);
	else
...
const struct v4l2_subdev_internal_ops aic_dvp_subdev_internal_ops = {
	.init_state	= aic_dvp_subdev_init_state,
};
