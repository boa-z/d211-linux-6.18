...
static int aic_dvp_notifier_init(struct aic_dvp *dvp)
{
	...
	v4l2_async_nf_init(&dvp->notifier, &dvp->v4l2);
	...
	dvp->asd = v4l2_async_nf_add_fwnode_remote(&dvp->notifier, ep,
...
static int aic_dvp_probe(struct platform_device *pdev)
{
	...
	subdev->internal_ops = &aic_dvp_subdev_internal_ops;
	...
	ret = v4l2_async_nf_register(&dvp->notifier);
	if (ret) {
...
static void aic_dvp_remove(struct platform_device *pdev)
{
	struct aic_dvp *dvp = platform_get_drvdata(pdev);

	v4l2_async_nf_unregister(&dvp->notifier);
	v4l2_async_nf_cleanup(&dvp->notifier);
	media_device_unregister(&dvp->mdev);
	aic_dvp_buf_unregister(dvp);
	media_device_cleanup(&dvp->mdev);
