...
struct aic_ir {
	...
	unsigned int	tx_resolution;	/* ns per RLC unit */
	int		irq;
...
static void aic_tx_fifo_clear(struct aic_ir *ir)
{
	unsigned int val;
...
static void aic_tx_int_enable(struct aic_ir *ir)
{
	unsigned int val;
...
static void aic_tx_int_disable(struct aic_ir *ir)
{
	unsigned int val;
...
static void aic_cir_tx_handle(struct aic_ir *ir)
{
	...
	for (i = ir->tx_idx; (i < ir->tx_size) && free_space; i++) {
		...
		duration = duration / ir->tx_resolution;
...
static irqreturn_t aic_ir_irq(int irqno, void *dev_id)
{
	...
	if (int_status & CIR_INTR_RX_OVF_INT) {
		ir_raw_event_set_idle(ir->rc, false);
		ir->rx_flag = 0;
...
static int aic_set_tx_carrier(struct rc_dev *rcdev, u32 carrier)
{
	...
	ir->tx_resolution = (US_PER_SEC / carrier);
	return 0;
}
...
static int aic_cir_probe(struct platform_device *pdev)
{
	...
	ir->tx_resolution = (US_PER_SEC / DEFAULT_FREQ);
	ir->rc->min_timeout = 1;
...
static void aic_cir_remove(struct platform_device *pdev)
{
	unsigned long flags;
