...
int aicmac_phy_init(void *priv_ptr)
{
	...
	phy_data->phylink_cfg.mac_capabilities = MAC_10 | MAC_100 | MAC_1000 |
						 MAC_SYM_PAUSE | MAC_ASYM_PAUSE;
	if (priv->plat->mac_data->max_speed &&
	    priv->plat->mac_data->max_speed < SPEED_1000)
		phy_data->phylink_cfg.mac_capabilities &= ~MAC_1000;
	if (priv->plat->tx_queues_to_use > 1)
		phy_data->phylink_cfg.mac_capabilities &=
			~(MAC_10HD | MAC_100HD | MAC_1000HD);
	phy_interface_zero(phy_data->phylink_cfg.supported_interfaces);
	__set_bit(mode, phy_data->phylink_cfg.supported_interfaces);
