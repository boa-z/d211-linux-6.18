...
{
	struct aic_tsen_dev *tsen = thermal_zone_device_priv(zone);
...
static void tsen_cpu_critical(struct thermal_zone_device *zone)
{
	struct aic_tsen_dev *tsen = thermal_zone_device_priv(zone);
...
static int aic_tsen_single_point_cali_for_correct(struct aic_tsen_dev *tsen)
{
	int i;
	int origin_vol;
	int origin_adc;
...
static int aic_tsen_single_point_cali(struct aic_tsen_dev *tsen)
{
	int i;
	int cali_scale;
	u32 *buf = tsen->cell_data;
...
static void aic_tsen_curve_fitting(struct aic_tsen_dev *tsen)
{
...
static int aic_tsen_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < tsen->ch_num; i++) {
		...
		/*
		 * The CPU zone exposes the calibrated high temperature alarm
		 * as a single CRITICAL trip.  The GPAI zone has no trip.
		 */
		if (i == AIC_TSEN_CH_CPU && chan->hta_enable) {
			chan->trips = devm_kcalloc(&pdev->dev, 1,
						   sizeof(*chan->trips),
						   GFP_KERNEL);
			if (!chan->trips) {
				ret = -ENOMEM;
			...
			chan->trips[0].type = THERMAL_TRIP_CRITICAL;
			chan->trips[0].temperature =
				tsen_data2temp(i, chan->hta_thd,
					       &tsen->data->ch[i]);
			num_trips = 1;
		}

		params[i]->offset = tsen->data->ch[i].offset;
		params[i]->slope = tsen->data->ch[i].slope;

		zone_dev = thermal_zone_device_register_with_trips(
							tsen->data->ch[i].name,
							chan->trips, num_trips,
							tsen, ops[i], params[i],
							0, 0);
		if (IS_ERR(zone_dev)) {
...
static void aic_tsen_exit(struct platform_device *pdev)
{
	int i;
