...
{
	static const char * const mode[] = {"Up", "Down", "UpDw"};
	static const char * const act[] = {"-", "Low", "Hgh", "Inv"};
...
static int aic_epwm_get_state(struct pwm_chip *chip, struct pwm_device *pwm,
...
static int aic_epwm_apply(struct pwm_chip *chip, struct pwm_device *pwm,
			  const struct pwm_state *state)
{
	...
	if (!epwm->args[ch].available) {
		dev_err(&chip->dev, "%s() ch%d is unavailable!\n", __func__, ch);
		return -ENODEV;
	}

	if (state->polarity != epwm->args[ch].polarity) {
		ret = aic_epwm_set_polarity(chip, pwm, state->polarity);
		if (ret)
			return ret;
	}
	...
	if (state->period != pwm->state.period ||
		...
		ret = aic_epwm_config(chip, pwm, state->duty_cycle,
				      state->period);
		if (ret)
			return ret;
	}
	...
	if (state->enabled != pwm->state.enabled) {
		if (state->enabled)
			ret = aic_epwm_enable(chip, pwm);
		else
			aic_epwm_disable(chip, pwm);
		if (ret)
			return ret;
	}
...
static int aic_epwm_probe(struct platform_device *pdev)
{
	...
	chip = devm_pwmchip_alloc(&pdev->dev, AIC_EPWM_CH_NUM, sizeof(*epwm));
	if (IS_ERR(chip))
		return PTR_ERR(chip);
	epwm = pwmchip_get_drvdata(chip);
	epwm->chip = chip;
	...
	ret = devm_pwmchip_add(&pdev->dev, epwm->chip);
	if (ret < 0) {
...
static void aic_epwm_remove(struct platform_device *pdev)
{
