			    ...
			    const char *buf, size_t len)
{
	...
	writel(PWM_ETSEL_INTEN | ((irq_mode + 4) << PWM_ETSEL_INTSEL_SHIFT),
	       apwm->regs + PWM_ETSEL(ch));
...
static int aic_pwm_get_state(struct pwm_chip *chip, struct pwm_device *pwm,
...
static int aic_pwm_apply(struct pwm_chip *chip, struct pwm_device *pwm,
			 const struct pwm_state *state)
{
	...
	if (!apwm->args[ch].available) {
		dev_err(&chip->dev, "%s() ch%d is unavailable!\n", __func__, ch);
		return -ENODEV;
	}

	if (state->polarity != apwm->args[ch].polarity) {
		ret = aic_pwm_set_polarity(chip, pwm, state->polarity);
		if (ret)
			return ret;
	}
	...
	if (state->period != pwm->state.period ||
		...
		ret = aic_pwm_config(chip, pwm, state->duty_cycle,
				     state->period);
		if (ret)
			return ret;
	}
	...
	if (state->enabled != pwm->state.enabled) {
		if (state->enabled)
			ret = aic_pwm_enable(chip, pwm);
		else
			aic_pwm_disable(chip, pwm);
		if (ret)
			return ret;
	}
	...
	for (i = 0; i < AIC_PWM_CH_NUM; i++) {
		if (stat & (1 << i)) {
			...
			if (isr_cnt[i] == g_pulse_para[i].pulse_cnt) {
				if ((g_pulse_para[i].irq_mode == PWM_CMPA_UP) ||
				    (g_pulse_para[i].irq_mode == PWM_CMPA_DOWN))
					aic_pwm_set_prd_duty(apwm, i, g_pulse_para[i].prd_ns,
							     g_pulse_para[i].prd_ns, PWM_SET_CMPA);
				if ((g_pulse_para[i].irq_mode == PWM_CMPB_UP) ||
				    (g_pulse_para[i].irq_mode == PWM_CMPB_DOWN))
					aic_pwm_set_prd_duty(apwm, i, g_pulse_para[i].prd_ns,
							     g_pulse_para[i].prd_ns, PWM_SET_CMPB);
...
static int aic_pwm_probe(struct platform_device *pdev)
{
	...
	chip = devm_pwmchip_alloc(&pdev->dev, AIC_PWM_CH_NUM, sizeof(*apwm));
	if (IS_ERR(chip))
		return PTR_ERR(chip);
	apwm = pwmchip_get_drvdata(chip);
	apwm->chip = chip;
	chip->ops = &aic_pwm_ops;
	...
	ret = devm_pwmchip_add(&pdev->dev, apwm->chip);
	if (ret < 0) {
...
static void aic_pwm_remove(struct platform_device *pdev)
{
