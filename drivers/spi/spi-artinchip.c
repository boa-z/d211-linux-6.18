			   ...
			   struct aic_spi_platform_data *pdata)
{
	...
	/* Same as U-Boot: set max DMA delay cycle */
	writel(readl(base_addr + SPI_REG_DCR) | 0x1f, base_addr + SPI_REG_DCR);
	spi_cltr_cfg_tc(SPI_MODE_0, base_addr);
	/*
	 * TCR BIT(10) (RPSM) is never programmed by this driver or U-Boot,
	 * but a set residue survives both the CMU hard reset and the GCR
	 * soft reset. With it set, every PIO read samples constant zeros
	 * while the transfer still completes without errors (seen as
	 * "unknown raw ID 0000000000" when the U-Boot handoff happened to
	 * leave it set). Drop it like the other sticky bits.
	 */
	writel(readl(base_addr + SPI_REG_TCR) & ~BIT(10),
	       base_addr + SPI_REG_TCR);
	/* Same as U-Boot set_mode(): program TX/RX phase for non-phase-ctl IP */
	writel(readl(base_addr + 0x0c) | BIT(12) | BIT(15), base_addr + 0x0c);
		...
		const char *buf, size_t count)
{
	...
	if (new_val != aicspi->freq_dividor) {
		...
		if (new_val == 0)
			freq = spi_ctlr_set_clk(clk_get_rate(aicspi->mclk),
						clk_get_rate(aicspi->mclk),
						base_addr);
		else
			freq = spi_ctlr_set_clk(clk_get_rate(aicspi->mclk) / new_val,
						clk_get_rate(aicspi->mclk),
						base_addr);

		spi_ctlr_set_rx_delay(aicspi, freq);
		dev_info(dev, "update freq: %u Hz, mclk: %lu Hz\n", freq,
			 clk_get_rate(aicspi->mclk));
...
static bool aic_spi_reinit_done;
...
static int aic_spi_mem_exec_op(struct spi_mem *mem, const struct spi_mem_op *op)
{
	...
	/*
	 * On D211 the controller does not drive the bus after the CMU hard
	 * reset until a GCR soft reset is issued (U-Boot does this in
	 * claim_bus()). The transfer immediately following the soft reset
	 * returns bogus data, so do the reset once here; the first op issued
	 * by the SPI NAND driver is a reset command without data phase.
	 */
	if (!aic_spi_reinit_done) {
		u8 idb[2];
		int i;

		aic_spi_reinit_done = true;
		spi_ctlr_enable_bus(base_addr);
		spi_ctlr_set_master_mode(base_addr);
		spi_ctlr_soft_reset(base_addr);
		udelay(10);
		...
		/*
		 * Wait until the NAND answers READ ID. The sticky high bits
		 * of TCR plus BIT(10) (RPSM, never programmed by this driver
		 * or U-Boot but surviving all resets) must be dropped before
		 * each transfer, otherwise the RX path returns zeros.
		 */
		for (i = 0; i < 50; i++) {
			struct spi_mem_op idop = {
				.cmd = SPI_MEM_OP_CMD(0x9f, 1),
				.addr = SPI_MEM_OP_NO_ADDR,
				.dummy = SPI_MEM_OP_NO_DUMMY,
				.data = SPI_MEM_OP_DATA_IN(2, idb, 1),
			};

			writel(readl(base_addr + SPI_REG_TCR) &
			       ~(GENMASK(25, 16) | BIT(10)),
			       base_addr + SPI_REG_TCR);

			idb[0] = 0;
			idb[1] = 0;
			aic_spi_mem_exec_op(mem, &idop);
			if (idb[0] == 0xef || idb[1] == 0xef)
				break;
			msleep(10);
		}
	}
	...
	if (aicspi->freq_dividor != 0)
		...
		aicspi->freq = aic_spi_set_clk(mem->spi->max_speed_hz,
					       clk_get_rate(aicspi->mclk), mem->spi);
...
static int aic_spi_probe(struct platform_device *pdev)
{
	...
	ctlr = spi_alloc_host(&pdev->dev, sizeof(struct aic_spi));
	if (ctlr == NULL) {
...
static void aic_spi_remove(struct platform_device *pdev)
{
	struct spi_controller *ctlr;
	struct resource	*mem_res;
	struct aic_spi *aicspi;
	...
	aic_spi_hw_exit(aicspi, aicspi->pdata);
