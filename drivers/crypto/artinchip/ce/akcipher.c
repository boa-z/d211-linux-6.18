...
void aic_akcipher_handle_irq(struct aic_crypto_dev *ce)
{
	...
	aic_akcipher_unprepare_req(ce->ak_accel.engine, &req->base);
...
static int aic_akcipher_do_one_req(struct crypto_engine *engine, void *areq)
{
	...
	ret = aic_akcipher_prepare_req(engine, areq);
	if (ret)
		return ret;
...
static int aic_akcipher_rsa_alg_init(struct crypto_akcipher *tfm)
{
	...
	akcipher_set_reqsize(tfm, sizeof(struct aic_akcipher_reqctx));
	return 0;
}
...
int aic_crypto_akcipher_accelerator_init(struct aic_crypto_dev *ce)
{
	...
	for (i = 0; i < ARRAY_SIZE(ak_algs); i++) {
		ak_algs[i].ce = ce;
		ak_algs[i].op.do_one_request = aic_akcipher_do_one_req;
		ret = crypto_engine_register_akcipher(
				(struct akcipher_engine_alg *)&ak_algs[i].alg);
		if (ret) {
