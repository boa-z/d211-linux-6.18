...
struct aic_skcipher_alg {
	...
	/*
	 * The crypto engine framework locates its ops right after the
	 * embedded crypto_alg, which is the last member of skcipher_alg.
	 */
...
static int aic_skcipher_do_one_req(struct crypto_engine *engine, void *areq)
{
	...
	ret = aic_skcipher_prepare_req(engine, areq);
	if (ret)
		return ret;
...
void aic_skcipher_handle_irq(struct aic_crypto_dev *ce)
{
	...
	aic_skcipher_unprepare_req(ce->sk_accel.engine, &req->base);
...
int aic_crypto_skcipher_accelerator_init(struct aic_crypto_dev *ce)
{
	...
	for (i = 0; i < ARRAY_SIZE(sk_algs); i++) {
		sk_algs[i].ce = ce;
		sk_algs[i].op.do_one_request = aic_skcipher_do_one_req;
		ret = crypto_engine_register_skcipher(
				(struct skcipher_engine_alg *)&sk_algs[i].alg);
		if (ret) {
