...
struct aic_hash_alg {
	...
	/* The engine ops must directly follow the embedded crypto_alg. */
...
static int aic_hash_do_one_req(struct crypto_engine *engine, void *areq)
{
	...
	ret = aic_hash_prepare_req(engine, areq);
	if (ret)
		return ret;

	if (!rctx->task) {
		/* Not enough data to start CE, just finalize current request */
		aic_hash_unprepare_req(ce->hash_accel.engine, areq);
...
void aic_hash_handle_irq(struct aic_crypto_dev *ce)
{
	...
	aic_hash_unprepare_req(ce->hash_accel.engine, &req->base);
...
static int aic_hash_alg_init(struct crypto_ahash *hash)
{
	struct aic_hash_tfm_ctx *ctx = crypto_ahash_ctx(hash);
	struct ahash_alg *halg = crypto_ahash_alg(hash);
	struct aic_hash_alg *aicalg;

	pr_debug("%s\n", __func__);
	crypto_ahash_set_reqsize(hash, sizeof(struct aic_hash_reqctx));
...
static int aic_hash_hmac_alg_init(struct crypto_ahash *hash)
{
	struct aic_hash_tfm_ctx *ctx = crypto_ahash_ctx(hash);
	int ret;

	ret = aic_hash_alg_init(hash);
	ctx->hmac = true;

	return ret;
}

static void aic_hash_alg_exit(struct crypto_ahash *hash)
{
	struct aic_hash_tfm_ctx *ctx = crypto_ahash_ctx(hash);
...
static struct aic_hash_alg hash_algs[] = {
{
	.alg = {
		...
		.init_tfm = aic_hash_alg_init,
		.exit_tfm = aic_hash_alg_exit,
		.halg = {
...
},
{
	.alg = {
		...
		.init_tfm = aic_hash_hmac_alg_init,
		.exit_tfm = aic_hash_alg_exit,
		.halg = {
			.digestsize = SHA1_DIGEST_SIZE,
			.statesize = sizeof(struct aic_hash_reqctx),
...
int aic_crypto_hash_accelerator_init(struct aic_crypto_dev *ce)
{
	...
	eng = crypto_engine_alloc_init_and_set(ce->dev, true, true,
	...
	for (i = 0; i < ARRAY_SIZE(hash_algs); i++) {
		hash_algs[i].ce = ce;
		hash_algs[i].op.do_one_request = aic_hash_do_one_req;
		ret = crypto_engine_register_ahash(
				(struct ahash_engine_alg *)&hash_algs[i].alg);
		if (ret) {
