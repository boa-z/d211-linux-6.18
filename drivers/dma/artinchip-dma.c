...
static inline void writel_clrbits(u32 mask, void __iomem *addr)
{
	writel(readl(addr) & ~mask, addr);
}

static inline void writel_bits(u32 val, u32 mask, u32 shift, void __iomem *addr)
{
	if (val)
...
static inline int readl_bit(u32 bit, void __iomem *addr)
{
	return (readl(addr) & bit) ? 1 : 0;
}
...
static void aic_dma_remove(struct platform_device *pdev)
{
