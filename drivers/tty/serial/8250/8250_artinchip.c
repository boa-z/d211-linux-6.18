...
static void aic8250_serial_out32(struct uart_port *p, unsigned int offset,
				 u32 value)
{
	struct aic8250_data *d = to_aic8250_data(p->private_data);
...
static u32 aic8250_serial_in32(struct uart_port *p, unsigned int offset)
{
				...
				const struct ktermios *old)
{
	...
	while (timeout--) {
		serial8250_clear_and_reinit_fifos(up);
		if ((serial_in(up, AIC_REG_UART_USR) & AIC_UART_USR_BUSY) == 0)
			break;
	}
...
static void aic8250_set_mctrl(struct uart_port *p, unsigned int mctrl)
{
	...
	mcr = (mcr & AIC_UART_MCR_FUNC_MASK) | up->mcr;
...
static void aic8250_remove(struct platform_device *pdev)
{
